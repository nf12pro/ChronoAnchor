...
@onready var gun_equipment = $gun_equipment
...
func _physics_process(delta: float) -> void:
	...
	if Global.on_windup:
		if Global.is_dashing and gun_equipment.heavy_gun_attack:
...
func dash() -> void:
	if Global.is_dashing or dash_charges <= 0:
		return 
	if Global.on_windup and gun_equipment.heavy_gun_attack:
		Global.on_windup = false
		Global.cancelled_attack = true
